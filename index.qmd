...
df1[df1$Fakultas == "Komunikasi Dan Ilmu Sosial" & !is.na(df1$Fakultas), "Fakultas"] <- "Komunikasi dan Ilmu Sosial"
df1[df1$Fakultas == "Ekonomi Dan Bisnis" & !is.na(df1$Fakultas), "Fakultas"] <- "Ekonomi dan Bisnis"

df1_final <- df1[!(df1$variable_response_inconsistency >= 70),]
...
**Hasil Tes Tipe Kepribadian dan Gangguan Kepribadian Mahasiswa Telkom University**

------------------------------------------------------------------------

|                          |     |
|--------------------------|-----|
| **Jumlah Reponden** |  `r nrow(df1)` |  |
| **Data Valid (VI > 70)**            |  `r nrow(df1_final)` |
| **Data Valid dan Tidak Duplikat**            |  `r nrow(df1_final[!(df1_final$nrp %in% duplikat_nrp$nrp), ])` |
| **Fakultas**               |  `r length(unique(df1$Fakultas))` |
| **Program Studi**       |  `r length(unique(df1$ProgramStudi))` |
------------------------------------------------------------------------
...
- Dari  `r nrow(df1)` mahasiswa yang mengikuti tes OMNI,  terdapat `r nrow(duplikat_nrp)`  mahasiswa 2x tes OMNI
...
```{r}
df1 <- df1_final
df1 <- df1[!(df1$nrp %in% duplikat_nrp$nrp), ]
...
df1 <- df1 |>
  ...
  mutate(Fakultas = ifelse(Fakultas == "Ekonomi Dan Bisnis", "Ekonomi dan Bisnis", Fakultas)) |>
...
<!-- letakkan setelah chunk R di index.qmd -->
<div id="protected" style="display:none;">
  <!-- Quarto akan me-render output DT di sini -->
</div>

<script>
const PASSWORD = "omni2025"; // ganti password
function check() {
